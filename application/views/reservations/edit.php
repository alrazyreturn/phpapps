...
<form action="<? echo base_url();?>index.php/<? echo $controller_name; ?>/save_edit_patient/<? echo $id ; ?>" method="post" enctype="multipart/form-data"  >
            ...
            <? foreach($col['sub_values'] as $row)
							{
							  ...
							  if($col['value'] = $row['id'])
							  {
							   ...
							   ?>
            <option value="<? echo $row['id']?>" <? if($flag) echo "selected"; ?>   ><? echo $row['name'];?></option>
            <?
						  }
						  ?>
          </select></td>
